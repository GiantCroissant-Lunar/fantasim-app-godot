...
/// <summary>
/// Godot-free payload parsing for the <c>render.mantle</c> command (DEPRECATED alias ingress,
/// directive 2 2026-07-16). Mirrors <see cref="CutawayRequest"/>: the T4 seam owns the Godot side
/// (wiring the parsed flag to the binder); this type owns the pure-C# parsing so it is unit-testable
/// without Godot. The parsed flag now drives the <c>geosphere.mantle</c> LAYER selection — not an
/// x-ray mode (see <see cref="MantleAlias"/>).
///
/// <para>Accepted payload: <c>{"enabled":true|false}</c>. An empty payload or a missing
/// <c>enabled</c> key activates the layer (the common case is "turn it on"); a non-boolean
