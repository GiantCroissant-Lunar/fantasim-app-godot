...
public static class SlabJointClassifier
{
        ...
        IReadOnlyDictionary<(int PlateA, int PlateB), SlabJointPolarity> polarity)
    {
        ...
        foreach (var kvp in byPair)
        {
            ...
            // The joint kind is the boundary TYPE from the existing source. All arcs of a pair share
            // one kind (the topology classifier emits one kind per plate pair); take the first and
            // assert the rest agree so a divergent input is never silently flattened.
            var kind = MapKind(pairArcs[0].Kind);
            for (int i = 1; i < pairArcs.Count; i++)
            {
                if (pairArcs[i].Kind != pairArcs[0].Kind)
                {
                    throw new InvalidOperationException(
                        $"Boundary arcs for plate pair ({lo}, {hi}) carry conflicting kinds: "
                        + $"{pairArcs[0].Kind} vs {pairArcs[i].Kind}. The topology classifier must "
                        + "emit one kind per plate pair.");
                }
            }
