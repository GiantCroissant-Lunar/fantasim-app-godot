...
| end-to-end windowed verify | NEXT | export complete-app, `remote__enabled=true`, `resource.reload_bundle` -> confirm `old ALC collected` in the log. The capstone that proves the whole fix (prior windowed verifies DISPROVED earlier attempts). |
